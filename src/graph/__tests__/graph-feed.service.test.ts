...
function targetId(protocol: string, chain: string): string {
    const target = LENDING_SUBGRAPHS.find(
        (candidate) =>
            candidate.protocol === protocol && candidate.chain === chain,
    );
...
function mockFetchByTarget(
    ...
    mockFetch.mockImplementation(async (input: unknown) => {
        ...
        const response = (target &&
            responses[`${target.protocol}|${target.chain}`]) ??
            responses.DEFAULT ?? { kind: "empty" };
...
describe("getStandardizedLendingBenchmarks", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        mockFetch.mockReset();
    });
    ...
    it("normalizes percentage and Ray-scale rates", async () => {
        ...
        const report = await getStandardizedLendingBenchmarks("test-api-key");
        expect(report.benchmarks.USDC).toBeDefined();
        expect(
            report.detailedRates.some((rate) => rate.supplyApy === 0.0363),
        ).toBe(true);
        expect(
            report.detailedRates.some((rate) => rate.supplyApy === 0.08),
        ).toBe(true);
    });
    ...
    it("maps bridged and Spark DAI symbols to canonical assets", async () => {
        ...
        const report = await getStandardizedLendingBenchmarks("test-api-key");
        expect(
            report.detailedRates.some((rate) => rate.symbol === "USDC"),
        ).toBe(true);
        expect(
            report.detailedRates.some(
                (rate) => rate.protocol === "Spark" && rate.symbol === "DAI",
            ),
        ).toBe(true);
    ...
    it("aggregates and deduplicates each canonical asset by deepest TVL", async () => {
        ...
        const report = await getStandardizedLendingBenchmarks("test-api-key");
        const usdc = report.detailedRates.filter(
            (rate) => rate.symbol === "USDC",
        );
        expect(usdc).toHaveLength(2);
        expect(
            usdc.find((rate) => rate.protocol === "Aave v3")?.supplyApy,
        ).toBe(0.04);
    ...
    it("filters non-stablecoins, inactive markets, and sub-million TVL", async () => {
        ...
        await expect(
            getStandardizedLendingBenchmarks("test-api-key"),
    ...
    it("reports missing canonical assets without fabricating fallbacks", async () => {
        ...
        await expect(
            ...
            return (
                error instanceof GraphFeedError &&
                error.code === "NO_LIVE_DATA" &&
                JSON.stringify(error.missingAssets) ===
                    JSON.stringify(["USDT", "DAI"])
            );
        });
    });
    ...
    it("returns ALL_TARGETS_FAILED for HTTP, GraphQL, and network failures", async () => {
        mockFetchByTarget({
            DEFAULT: { kind: "reject", message: "ECONNREFUSED" },
        });
        await expect(
            getStandardizedLendingBenchmarks("test-api-key"),
        ).rejects.toMatchObject({
        ...
        mockFetchByTarget({
            DEFAULT: { kind: "graphql-error", message: "rate limited" },
        });
        await expect(
            getStandardizedLendingBenchmarks("test-api-key"),
        ).rejects.toMatchObject({
    ...
    it("preserves metadata and uses the configured API key", async () => {
        ...
        const report = await getStandardizedLendingBenchmarks("key-for-test");
        const rate = report.detailedRates.find(
            (item) => item.marketName === "USDCn",
        );
    ...
    it("keeps the shared query constrained to the three stablecoins", () => {
        ...
        expect(
            LENDING_SUBGRAPHS.some((target) => target.protocol === "Spark"),
        ).toBe(true);
    });
});
