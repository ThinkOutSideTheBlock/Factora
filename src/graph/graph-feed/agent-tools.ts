...
export async function querySubgraph(
    ...
    try {
        const data = await graphMcpClient.queryDynamic(
            subgraphId,
            query,
            variables,
        );
                ...
                rawResults.map((result) => ({
                    ...
                    currentDeploymentIpfsHash:
                        typeof result?.currentDeploymentIpfsHash === "string"
                            ...
                            : typeof result?.currentVersion?.subgraphDeployment
                                    ?.ipfsHash === "string"
                              ? result.currentVersion.subgraphDeployment
                                    .ipfsHash
                              : null,
                })),
            ),
...
export async function getDynamicYieldOpportunities(
    ...
    try {
        return {
            opportunities:
                await mcpMarketService.getDynamicYieldOpportunities(query),
            isError: false,
        };
    } catch (error) {
        return {
 ...
 */
function dedupeBySubgraphId<T extends { subgraphId: string }>(
    results: T[],
): T[] {
 ...
 * `get_dex_liquidity_yield` are backed by the functional Messari feed (Engine A / Engine
